...
ind = landmarks(X, k = 25)

p = figure(width=350, height=350)
p.scatter(*X.T, color='lightblue', size=4)
p.scatter(*X[ind].T, color='red', size=6, line_color='gray')
show(p)

# %% Show packing / coverage guarentees 
ind, radii = landmarks(X, k = 40, radii = True)

## Show coverage of the union 
p = figure(width=350, height=350, title="Coverage of landmarks")
...
q = figure(width=350, height=350, title="Packing of landmarks")
...
ind = landmarks(X_dist, k = 40)

## Show different metrics
p = figure(width=350, height=350, title="Landmarks with L1 metric")
p.scatter(*X.T, color='lightblue', size=3, line_color='black', line_width=0.5)
p.scatter(*X[ind].T, color='red', size=6, line_color='white')
show(p)
