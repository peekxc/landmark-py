...
py::tuple maxmin_pc(
  ...
  auto ra_arr = py::array_t< double >(cover_radii.size(), cover_radii.data());
	return(py::make_tuple(lm_arr, ra_arr));
}
...
py::tuple maxmin(const py::array_t< double >& x, const double eps, const size_t n, const size_t n_pts, bool pairwise_dist, int seed){
	if (pairwise_dist){
    if (x.ndim() != 1){ throw std::invalid_argument("Distance input must be a two dimensional array."); }
    ...
    const size_t d = size_t(x.shape(1));
		return(maxmin_pc(X_ptr, eps, n, n_pts, d, 1, seed, 0));
	}
}
