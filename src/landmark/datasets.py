...
def load_shape(shape: str) -> np.ndarray:
  """Loads a specific shape dataset from the shape sets in the 'Clustering basic benchmark' by P. Fänti and S. Sieranoja."""
  assert shape.lower() in _SHAPE_DATA_URLS.keys(), f"Invalid shape name {shape} given; must be one of {list(_SHAPE_DATA_URLS.keys())}"
